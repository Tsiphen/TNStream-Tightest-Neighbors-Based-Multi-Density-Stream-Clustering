...
from BT_TNStream import TNStream # import class MCMSTStream from the same directory
import time
...
while T < 1000:
    ...
    start = time.time()    
    kds=TNStream(X,labels_true,N,W,r,n_micro,X.shape[1],plotFigure,4,mk,mk-1)
    end = time.time()
    ...
    if Purity>=0.95:
        ...
        time.sleep(2)
        if X.shape[1]==2:
            kds.plotGraph(str("BT-TNStream"))
        elif X.shape[1]==3:
            kds.plotGraph3D_normal(str("BT-TNStream"))